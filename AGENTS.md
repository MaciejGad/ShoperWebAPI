...
- Some resources genuinely have **no update or delete endpoint** — check the OpenAPI paths, not
  just the schema list. `LoyaltyEvent` only documents `GET /loyalty-events` (list), `POST
